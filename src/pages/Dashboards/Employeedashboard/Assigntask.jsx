...
const initialData = {
  ...
  tasks: {},
  columnOrder: ["Open", "In Progress", "Review", "Completed"],
};
...
function Droppable({ id, children, data, clearDeletedTasks }) {
  ...
  const isDeletedColumn = id === "Deleted";
  const isCompletedColumn = id === "Completed"; // Identify if it's the Completed column
  ...
  return (
    <div
      ...
      } ${isCompletedColumn ? "opacity-50 pointer-events-none" : ""}`} // Apply opacity and disable pointer events for Completed column
    >
      {children}
...
export default function Assigntask() {
  ...
  const handleDragEnd = (event) => {
    ...
    if (cardId !== overId) {
      ...
      if (overId === "Completed") {
        toast.error(`Moving tasks to 'Completed' is not allowed.`);
        return; // Prevent the move by returning early
      }
